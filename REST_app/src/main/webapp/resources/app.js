...
function display_books(data)
{
	var out_data="";
	 ...
	 for(var  i in data) {
		 var book = data[i];
		 out_data = out_data + "<tr><td>pealkiri:</td><td bgcolor=ffffff>" + book.title + "</td><td>autor:</td><td bgcolor=ffffff>" + book.author + "</td>";
...
function add_book()
{
	...
	$.ajax({
		...
		success: function(data) {
			...
			console.log(JSON.stringify(data));
			// uuendame raamatute nimekirja vormil , yks raamat on nyyd lisatud
			get_books();
 }
 });
}
